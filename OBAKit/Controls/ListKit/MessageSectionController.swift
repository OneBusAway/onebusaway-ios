...
final class MessageCell: BaseSelfSizingTableCell {
    ...
    private func configureView() {
        ...
        self.topStack.axis = isAccessibility ? .vertical : .horizontal
        self.authorLabel.numberOfLines = isAccessibility ? 3 : 1

        let subjectNumberOfLines = data.subjectNumberOfLines
        if subjectNumberOfLines > 0 {
            self.subjectLabel.numberOfLines = isAccessibility ? subjectNumberOfLines * 3 : subjectNumberOfLines
        } else {
            self.subjectLabel.numberOfLines = subjectNumberOfLines
        }

        let summaryNumberOfLines = data.summaryNumberOfLines
        if summaryNumberOfLines > 0 {
            self.summaryLabel.numberOfLines = isAccessibility ? summaryNumberOfLines * 4 : summaryNumberOfLines
        } else {
            self.summaryLabel.numberOfLines = summaryNumberOfLines
        }
