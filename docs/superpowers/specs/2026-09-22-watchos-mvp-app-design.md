...
`authorizationStatus: CLAuthorizationStatus`. It republishes the provider's
service as observable `apiService: RESTAPIService?` and exposes
...
3. **`StandaloneAPIServiceProvider`** — `@MainActor final class: NSObject,
   ...
   `private(set) var apiService: RESTAPIService?` and `var onChange: (() -> Void)?`.
   ...
   nil when `currentRegion` is nil. No `@Observable` in core; the host
   republishes.
